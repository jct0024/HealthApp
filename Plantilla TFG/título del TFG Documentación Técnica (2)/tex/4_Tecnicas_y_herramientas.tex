...
\subsection{Calculos, Administración y vista}
...
\begin{equation}
DiferenciaMacronutriente = (KMCD - A)/((KCDT-KMM)+(KMTC-KTM))
\end{equation}
Donde:
...
\begin{equation}
DiferenciaTotal = (DiferenciaHidratos+DiferenciaProteina+DiferenciaGrasas)/3
\end{equation} 
...
Para comenzar, he de transmitir que no estoy conforme al completo respecto a que al calculo de los gráficos este dentro de este módulo, pero debido al estado avanzado del proyecto, y la existencia de tres módulos, que siguen un patrón, base de datos, vista y cálculos, se decidió no romper esta estructura, y añadir estos cálculos a calculo dieta, y dividir los grafos en su parte visual (Que veremos a posteriori en el modulo VISTA) y su parte de cálculo1.\\
...
En este apartado hablaremos de como esta estructurada la base de datos, y de como se cargan, se guardan y se trabajan con los datos desde el módulo AdminBase, el cual, es un modulo bastante simple, necesario para el trato de datos.
...
\imagen{BasePatologias}{Estructura base de datos Patologías}
